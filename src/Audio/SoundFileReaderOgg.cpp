...
namespace
{
	size_t read(void* ptr, size_t size, size_t nmemb, void* data){
		Audio::InputStream* stream = static_cast<Audio::InputStream*>(data);
...
bool SoundFileReaderOgg::open(InputStream& stream, Info& info){
	...
	if(status < 0){
		cerr<< "Failed to open Vorbis file for reading" << std::endl;
		return false;
	}
